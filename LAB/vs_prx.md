...
| 2 | **PIERROT 0.8B** | phase2 step **2.50M** | 약 0.857B | 16 |
| 3 | **PIERROT 0.8B** | phase2 step **2.60M** | 약 0.857B | 16 |
| 4 | **PIERROT 0.8B** | phase2 step **2.745M** | 약 0.857B | 16 |
| 5 | **PIERROT 1.6B** | phase3 step **800k** | 약 1.6B | 33 |
| 6 | **PIERROT 1.6B** | phase3 step **850k** | 약 1.6B | 33 |
| 7 | **PIERROT 1.6B** | phase3 step **930k** | 약 1.6B | 33 |

공통 구성은 다음과 같다.

| 항목 | PIERROT (6개 전부) | PRX |
| --- | --- | --- |
...
1.6B는 0.8B에서 레이어를 늘려 이어 학습한 모델이라([1.6b_training_review.md](1.6b_training_review.md) 3.0절), 여섯은 하나의 계보 위에 있다. 텍스트 인코더는 PIERROT 여섯이 모두 같고, **PRX만 T5Gemma 2.61B로 다르다.**
...
| 3 | `0.8B 2.60M` | PIERROT 0.8B · phase2 step 2,600,000 |
| 4 | `0.8B 2.745M` | PIERROT 0.8B · phase2 step 2,745,000 |
| 5 | `1.6B 800k` | PIERROT 1.6B · phase3 step 800,000 |
| 6 | `1.6B 850k` | PIERROT 1.6B · phase3 step 850,000 |
| 7 | `1.6B 930k` | PIERROT 1.6B · phase3 step 930,000 |

2~4번 줄이 0.8B의 세 시점, 5~7번 줄이 1.6B의 세 시점이다. **가로로 읽으면 모델 간 비교, 세로로 읽으면 학습 진행에 따른 변화**가 된다.
...
> | **7모델** | `vs_prx_sheetN_7models_20260720.jpg` | + 0.8B 2.60M, 1.6B 850k **(아래 게재본)** |

### 3.1 인물 · 손 · 개수 · 공간 (id 1–4)

![PRX vs PIERROT 6 checkpoints 1](../docs/vs_prx_sheet1_7models_20260720.jpg)


### 3.2 속성 결합 · 텍스트 · 정물 (id 5–8)

![PRX vs PIERROT 6 checkpoints 2](../docs/vs_prx_sheet2_7models_20260720.jpg)


### 3.3 투명 · 반사 · 야간 · 모션 (id 9–12)

![PRX vs PIERROT 6 checkpoints 3](../docs/vs_prx_sheet3_7models_20260720.jpg)


### 3.4 군중 · 건축 · 음식 · 차량 (id 13–16)

![PRX vs PIERROT 6 checkpoints 4](../docs/vs_prx_sheet4_7models_20260720.jpg)


### 3.5 일러스트 · 애니 · SF · 매크로 (id 17–20)

![PRX vs PIERROT 6 checkpoints 5](../docs/vs_prx_sheet5_7models_20260720.jpg)
...
- **속성 결합(attribute binding)이 강하다.** 여러 인물에게 각기 다른 색 옷과 소지품을 지정한 프롬프트(id 5)에서 여섯 체크포인트 모두 노란 우비·파란 우산·초록 스웨터·빨간 보온병을 정확히 배치했고, 인물도 정면으로 세웠다. PRX는 두 인물을 뒷모습으로 그려 누가 무엇을 들었는지 모호했다.
...
- **동작·순간 포착이 약하다.** 가장 뚜렷한 약점이다. "공중에 뜬"(id 12 프리스비), "드리프트 중인"(id 16 랠리카)을 **여섯 체크포인트가 전부** 무시하고 정적인 장면을 그렸다. PRX는 둘 다 순간을 포착했다. 서로 다른 프롬프트가 같은 방향으로, 그리고 체급·학습량과 무관하게 실패했다.
- **정확한 구조가 필요한 곳에서 무너진다.** 천창에서 떨어지는 빛의 기하(id 14)는 여섯 체크포인트 모두 아치 통로로 바뀌었고, 나비 비늘(id 20)은 표본 사진처럼 평평하다.
...
- **미세 질감의 사실성이 부족하다.** 정동석 결정(id 8)은 여섯 체크포인트 모두 매끈한 보석처럼 나왔고, 라멘(id 15)은 면발이 보이지 않는 스튜에 가깝다. PRX가 확실히 앞선다.
...
| 0.8B 2.60M | `MORNING TOORLOFF` — 앞 단어만 맞음 | `DEEPEK / DEEPEKOWOREK` — 판독 불가 |
...
시점을 여섯으로 늘리니 **네 단계를 밟아 올라가는 과정**이 그대로 보인다.

1. **판독 불가**(2.50M) — 글자 비슷한 형태만 만든다.
2. **앞 단어만 맞음**(2.60M) — `MORNING`은 맞추지만 뒷 단어가 무너진다. 긴 문자열의 앞쪽부터 자리를 잡는다.
3. **거의 맞되 글자 중복**(2.745M) — 단어를 알아보되 `LOAFF`, `WORRK`처럼 글자를 하나씩 덧붙인다.
4. **정확**(1.6B 800k·850k) — 두 문구 모두 철자까지 맞는다.

따옴표 안 텍스트를 글자 단위로 토큰화하는 처리가, 학습량이 쌓이면서 단계적으로 작동하기 시작했다고 볼 수 있다. 특히 **800k와 850k가 연속으로 정확한 것**은 우연이 아니라 능력이 자리 잡았다는 근거가 된다.

다만 **930k에서 긴 문구가 다시 틀린 것**(`MORING LOAF`)은 주의할 지점이다. 800k·850k가 맞췄는데 930k만 틀렸으므로, 단조롭게 좋아지기만 하는 것이 아니라 **후반에 흔들리는 구간이 있다.** 글자 수를 4·8·12·16자로 늘려 가며 여러 시점에서 측정하면 경계와 흔들림의 폭을 잡을 수 있다.
...
**개수 세기(id 3)도 흔들린다.** 요청은 오리 3개인데, 2.745M·850k·930k는 3개로 맞췄지만 **2.50M은 3개, 2.60M은 5개, 800k는 4개**로 시점마다 달라진다. 학습량이 많다고 항상 정확해지는 것이 아니며, 개수 세기는 아직 안정적으로 자리 잡지 않은 능력으로 보인다.
...
- **얻은 것**: 텍스트 정확도(5.3)가 가장 뚜렷하다. 0.8B는 세 시점 모두 철자를 틀렸지만 1.6B는 800k·850k에서 정확해졌다. 인물 표현의 정돈도도 올랐다. 지하철 승강장(id 13)의 공간 깊이감도 1.6B 쪽이 낫다.
...
프롬프트와 조건은 [vs_prx_prompts.json](vs_prx_prompts.json)에 있다. PRX는 `Photoroom/prx-1024-t2i-beta`를 diffusers로 불러 `num_inference_steps=28, guidance_scale=5.0, seed=42`로, PIERROT는 phase2 step 2.50M·2.60M·2.745M(0.8B)과 phase3 step 800k·850k·930k(1.6B)의 EMA 체크포인트를 각각 `28 steps, guidance_scale=4.0, seed=42, chi_prompt OFF`로 생성했다. 일곱 모델 모두 1024²·bf16이다.
