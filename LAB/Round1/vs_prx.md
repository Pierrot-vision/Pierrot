...
| 2 | **PIERROT 0.8B** | phase2 step **2.00M** | 약 0.857B | 16 |
| 3 | **PIERROT 0.8B** | phase2 step **2.50M** | 약 0.857B | 16 |
| 4 | **PIERROT 0.8B** | phase2 step **2.60M** | 약 0.857B | 16 |
| 5 | **PIERROT 0.8B** | phase2 step **2.745M** | 약 0.857B | 16 |
| 6 | **PIERROT 1.6B** | phase3 step **700k** | 약 1.6B | 33 |
| 7 | **PIERROT 1.6B** | phase3 step **750k** | 약 1.6B | 33 |
| 8 | **PIERROT 1.6B** | phase3 step **800k** | 약 1.6B | 33 |
| 9 | **PIERROT 1.6B** | phase3 step **850k** | 약 1.6B | 33 |
| 10 | **PIERROT 1.6B** | phase3 step **900k** | 약 1.6B | 33 |
| 11 | **PIERROT 1.6B** | phase3 step **930k** | 약 1.6B | 33 |

공통 구성은 다음과 같다.

| 항목 | PIERROT (10개 전부) | PRX |
| --- | --- | --- |
...
1.6B는 0.8B에서 레이어를 늘려 이어 학습한 모델이라([1.6b_training_review.md](1.6b_training_review.md) 3.0절), 열은 하나의 계보 위에 있다. 텍스트 인코더는 PIERROT 열이 모두 같고, **PRX만 T5Gemma 2.61B로 다르다.**
...
| 2 | `0.8B 2.00M` | PIERROT 0.8B · phase2 step 2,000,000 |
| 3 | `0.8B 2.50M` | PIERROT 0.8B · phase2 step 2,500,000 |
| 4 | `0.8B 2.60M` | PIERROT 0.8B · phase2 step 2,600,000 |
| 5 | `0.8B 2.745M` | PIERROT 0.8B · phase2 step 2,745,000 |
| 6 | `1.6B 700k` | PIERROT 1.6B · phase3 step 700,000 |
| 7 | `1.6B 750k` | PIERROT 1.6B · phase3 step 750,000 |
| 8 | `1.6B 800k` | PIERROT 1.6B · phase3 step 800,000 |
| 9 | `1.6B 850k` | PIERROT 1.6B · phase3 step 850,000 |
| 10 | `1.6B 900k` | PIERROT 1.6B · phase3 step 900,000 |
| 11 | `1.6B 930k` | PIERROT 1.6B · phase3 step 930,000 |

2~5번 줄이 0.8B의 네 시점, 6~11번 줄이 1.6B의 여섯 시점이다. **가로로 읽으면 모델 간 비교, 세로로 읽으면 학습 진행에 따른 변화**가 된다.
...
> | 5모델 | `vs_prx_sheetN_5models_20260720.jpg` | + 1.6B 800k |
> | 7모델 | `vs_prx_sheetN_7models_20260720.jpg` | + 0.8B 2.60M, 1.6B 850k |
> | 9모델 | `vs_prx_sheetN_9models_20260720.jpg` | + 0.8B 2.00M, 1.6B 900k |
> | **11모델** | `vs_prx_sheetN_11models_20260720.jpg` | + 1.6B 700k·750k **(아래 게재본)** |

### 3.1 인물 · 손 · 개수 · 공간 (id 1–4)

![PRX vs PIERROT 10 checkpoints 1](../../docs/vs_prx_sheet1_11models_20260720.jpg)


### 3.2 속성 결합 · 텍스트 · 정물 (id 5–8)

![PRX vs PIERROT 10 checkpoints 2](../../docs/vs_prx_sheet2_11models_20260720.jpg)


### 3.3 투명 · 반사 · 야간 · 모션 (id 9–12)

![PRX vs PIERROT 10 checkpoints 3](../../docs/vs_prx_sheet3_11models_20260720.jpg)


### 3.4 군중 · 건축 · 음식 · 차량 (id 13–16)

![PRX vs PIERROT 10 checkpoints 4](../../docs/vs_prx_sheet4_11models_20260720.jpg)


### 3.5 일러스트 · 애니 · SF · 매크로 (id 17–20)

![PRX vs PIERROT 10 checkpoints 5](../../docs/vs_prx_sheet5_11models_20260720.jpg)
...
- **속성 결합(attribute binding)이 강하다.** 여러 인물에게 각기 다른 색 옷과 소지품을 지정한 프롬프트(id 5)에서 열 체크포인트 모두 노란 우비·파란 우산·초록 스웨터·빨간 보온병을 정확히 배치했고, 인물도 정면으로 세웠다. PRX는 두 인물을 뒷모습으로 그려 누가 무엇을 들었는지 모호했다.
...
- **동작·순간 포착이 약하다.** 가장 뚜렷한 약점이다. "공중에 뜬"(id 12 프리스비), "드리프트 중인"(id 16 랠리카)을 **열 체크포인트가 전부** 무시하고 정적인 장면을 그렸다. PRX는 둘 다 순간을 포착했다. 서로 다른 프롬프트가 같은 방향으로, 그리고 체급·학습량과 무관하게 실패했다.
- **정확한 구조가 필요한 곳에서 무너진다.** 천창에서 떨어지는 빛의 기하(id 14)는 열 체크포인트 모두 아치 통로로 바뀌었고, 나비 비늘(id 20)은 표본 사진처럼 평평하다.
...
- **미세 질감의 사실성이 부족하다.** 정동석 결정(id 8)은 열 체크포인트 모두 매끈한 보석처럼 나왔고, 라멘(id 15)은 면발이 보이지 않는 스튜에 가깝다. PRX가 확실히 앞선다.

### 5.3 학습이 진행되며 좋아진 것 — 텍스트 렌더링

열 시점을 나란히 보면 **글자 쓰기 능력이 자라고, 정점을 찍고, 다시 흔들리는 전 과정**이 한 화면에 들어온다.

| 체크포인트 | id 6 "MORNING LOAF" | id 7 "DEEP WORK" | 단계 |
| --- | --- | --- | --- |
| 0.8B 2.00M | `IMORISNNG LOR NNIIIG` | `OO EPPP / DEEP EORKOM` | 판독 불가 |
| 0.8B 2.50M | `MORNIING IOODINE` | `DEEPTN / RD EPFFROWORK` | 판독 불가 |
| 0.8B 2.60M | `MORNING TOORLOFF` | `DEEPEK / DEEPEKOWOREK` | 앞 단어만 |
| 0.8B 2.745M | `MORNING LOAFF` | `DEEP WORRK` | 글자 중복 |
| 1.6B 700k | **정확** | **정확** | **정점** |
| 1.6B 750k | **정확** | `DEEPP WORK` — P 중복 | 흔들림 |
| 1.6B 800k | **정확** | **정확** | **정점** |
| 1.6B 850k | **정확** | **정확** | **정점** |
| 1.6B 900k | **정확** | `DEEEP WORK` — E 중복 | 흔들림 |
| 1.6B 930k | `MORING LOAF` — N 누락 | **정확** | 흔들림 |
| (참고) PRX | 정확 | 뭉개져 판독 불가 | — |

**성장 구간(0.8B)** 은 네 단계를 밟는다.

1. **판독 불가**(2.00M·2.50M) — 글자 비슷한 형태만 만든다.
2. **앞 단어만 맞음**(2.60M) — `MORNING`은 맞추지만 뒷 단어가 무너진다. 긴 문자열의 **앞쪽부터 자리를 잡는다.**
3. **글자 중복**(2.745M) — 단어를 알아보되 `LOAFF`, `WORRK`처럼 글자를 하나씩 덧붙인다.
4. **정확**(1.6B 700k~) — 철자까지 맞는다.

**정점과 흔들림(1.6B)** 이 더 흥미롭다. 700k에서 이미 두 문구 모두 정확해진 뒤, 750k·900k·930k에서 산발적으로 틀린다. 틀리는 방식도 일정하지 않다 — `DEEPP`(P 중복), `DEEEP`(E 중복), `MORING`(N 누락).

정리하면 **단조 개선이 아니라 "700k 무렵 능력 획득 → 이후 시점마다 오락가락"** 이다. 800k·850k만 봤다면 "완성됐다"고 결론 냈겠지만, 앞뒤를 채우니 **아직 안정화되지 않은 상태**임이 드러난다. 체크포인트를 촘촘히 뽑아야 하는 이유가 이것이다.

한 가지 더: 틀릴 때 **두 문구가 동시에 틀리지 않는다.** 750k·900k는 짧은 문구만, 930k는 긴 문구만 틀렸다. 특정 시점이 "글자를 못 쓰게 되는" 것이 아니라, 매번 다른 지점에서 미끄러지는 성격에 가깝다.
...
**개수 세기(id 3)도 흔들린다.** 요청은 오리 3개인데 시점마다 3~5개로 달라진다(2.60M은 5개, 800k는 4개). 학습량이 많다고 항상 정확해지는 것이 아니며, 개수 세기는 아직 안정적으로 자리 잡지 않은 능력으로 보인다.
...
- **얻은 것**: 텍스트 정확도(5.3)가 가장 뚜렷하다. 0.8B는 네 시점 모두 철자를 틀렸지만, 1.6B는 700k부터 정확해졌다. 인물 표현의 정돈도도 올랐다. 지하철 승강장(id 13)의 공간 깊이감도 1.6B 쪽이 낫다.
...
프롬프트와 조건은 [vs_prx_prompts.json](../vs_prx_prompts.json)에 있다. PRX는 `Photoroom/prx-1024-t2i-beta`를 diffusers로 불러 `num_inference_steps=28, guidance_scale=5.0, seed=42`로, PIERROT는 phase2 step 2.00M·2.50M·2.60M·2.745M(0.8B)과 phase3 step 700k·750k·800k·850k·900k·930k(1.6B)의 EMA 체크포인트를 각각 `28 steps, guidance_scale=4.0, seed=42, chi_prompt OFF`로 생성했다. 열한 모델 모두 1024²·bf16이다.
