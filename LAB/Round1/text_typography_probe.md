...
이름은 여섯 개지만 **실제 이미지는 그만큼 있지 않다. 상당수가 같은 그림을 다시 등록한 것이고, 그만큼 중복 노출된다.**

| 데이터셋 | 이미지 | 정체 | 노출 |
| --- | --- | --- | --- |
| `ovis_image_text_synth` | **129,991** | 원본 | 2.88% |
| `ovis_image_text_hard_synth` | 80,104 | ↑ 의 **100% 부분집합** (어려운 철자만) | 2.94% |
| `ovis_image_text_synth_longspell` | 8,206 | ↑ 의 부분집합 (이중자 + 7자 이상) | 2.46% |
| `ovis_image_text_hard_synth_longspell` | 5,906 | ↑ 의 부분집합 (같은 필터) | 2.46% |
| `ovis_scene_text_confusable_synth` | 6,772 | **별도 생성** (혼동 글자쌍) | 2.38% |
| `ovis_scene_text_l1_synth_ocr` | 14,926 | **별도 생성** (난이도 1단계) | 2.88% |

`hard_synth` 80,104장이 원본에 **100% 포함**된다는 것은 sample_id 대조로 확인했다. `longspell` 둘은 별도 폴더조차 없이 `converted_longspell/` 경로의 필터 뷰다.

**고유 이미지는 원본 129,991 + 별도 생성 21,698, 합쳐 15만 장 남짓**이다. 여섯 줄을 더한 24만이 아니다.

그런데 **여섯 개가 전부 따로 등록돼 각각 노출을 받는다.** 그래서 원본 13만 장 중 어려운 것들은 실제로 여러 번 뽑힌다.

| 이미지 종류 | 몇 번 등록됐나 | 합산 노출 |
| --- | --- | --- |
| 이중자 + 어려운 철자 (5,906장) | 4회 (synth · hard · longspell · hard_longspell) | **10.7%** |
| 어려운 철자 (나머지 74,198장) | 2회 (synth · hard) | 5.8% |
| 평범한 것 (49,887장) | 1회 | 2.88% |

**같은 그림 5,906장이 전체 학습의 10.7%를 차지한다.** 이것이 의도한 설계이긴 하다 — 어려운 샘플을 더 자주 보여주려는 것이다. 하지만 대가가 있다.

- **다양성은 늘지 않는다.** 같은 이미지·같은 문구를 더 보는 것이지 새 글자 조합을 보는 것이 아니다. 원본이 106개 단어에 갇혀 있으므로(1절) 몇 번을 반복해도 **어휘는 그대로**다.
- **과적합 위험이 있다.** 5,906장을 10% 비중으로 반복하면 그 특정 이미지·문구에 맞춰질 뿐, 새 단어로 일반화된다는 보장이 없다.
- **표를 그냥 읽으면 규모를 오해한다.** 여섯 줄 × 2~3% = 16%가 텍스트에 쓰인 것은 맞지만, 그것이 **16%만큼의 서로 다른 그림**을 의미하지는 않는다.

> **참고 — 디스크 용량도 같은 이유로 부풀어 있다.** `ovis_image_text_synth` 폴더의 parquet 195개는 `converted_multicap`(33.2GB, 원본 화질) · `converted_q85`(16.2GB, JPEG 85 재압축) · `converted_longspell`(2.0GB, 필터)로 나뉜다. q85와 multicap은 sample_id가 129,991개 완전히 일치하고, 디코딩해 픽셀을 비교하면 차이 0.19/255로 **같은 이미지**다. 즉 51GB는 13만 장을 세 벌로 들고 있는 것이다.
