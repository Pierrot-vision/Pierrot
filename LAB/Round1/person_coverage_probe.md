...
- **데이터셋 하나만 보고 판단하면 안 된다.** 인종은 이 학습셋에 0%였지만 다른 데이터가 메워 줬다. 보강 계획은 전체 학습셋을 합쳐서 세워야 한다.
- **체형은 의도적으로 채워야 한다.** 인종과 달리 되돌아갔다 — 웹 이미지 전반이 같은 편향을 공유하기 때문이다. [round1_experiment_report.md](round1_experiment_report.md) 6절 ④의 생성 대상에 체형을 명시해야 한다.
- **인접 개념이 있으면 전이된다.** `rolled`(7.4%)가 있어 `cuffed`는 됐고, 없는 `fold`는 실패했다. 새 개념은 0에서 시작하지 않는 편이 낫다.
