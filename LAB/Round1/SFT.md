...
다만 이것은 `3e-5`가 `1e-4`보다 항상 좋다는 결론이 아니다. 리소스 부족으로 `1e-4`와 `3e-5`를 같은 조건에서 충분히 길게 비교하는 ablation은 하지 못했다. 우리는 초기 `1e-4` 실험에서 base capability overwrite 위험을 보았고, 그 위험을 줄이기 위해 보수적으로 LR을 낮춘 것이다. 따라서 최종적으로 어느 LR이 더 좋은지는 별도 실험이 필요하다. 이 문서에서는 `1e-4`와 `3e-5`의 우열을 결론내리지 않는다.
