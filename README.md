...
**PIERROT** 는 1인 **텍스트→이미지(Text-to-Image, T2I)** 생성 연구·개발 프로젝트입니다. 거대 자원 없이도 작은 모델로 어디까지 갈 수 있는지 실험하는 개인 테스트베드이며, 다음 네 가지를 설계 철학으로 둡니다.
...
- **참여·조언·채용 등 관련 모든 문의는 언제나 환영입니다.**
...
딥러닝(Deep Learning) 분야에서는 오래 일해왔지만, Diffusion 을 본격적으로 다룬 지는 약 1.5년쯤 됩니다. 그동안은 주로 LoRA 기반의 도메인 특화(domain-specific) 학습만 반복해 왔는데, 솔직히 조금 지겨웠습니다. 그사이 세상에는 빠르게 바뀌는 흥미로운 연구들이 쏟아졌고요.

미국, 특히 중국 회사들은 꾸준히 괜찮은 모델을 내놓는데 "왜 우리나라 회사들은 잘 시도하지 않을까?" 하는 궁금증이 있었습니다. 거기서 한 걸음 더 나아가 "스크래치(from scratch) 학습이 정말 그렇게 어려운 일일까?" 가 궁금해졌고, 무엇보다 — 나도 한 번 바닥부터 직접 나만의 모델을 만들어 학습해 보고 싶다는 욕구가 컸습니다.
